...
namespace model
{
	namespace test
	{
		...
		TEST_F( FrustumTest, CompleteModelViewProjection )
		{
			...
			// The boxes here are given in world coordinates.
			// 1st case: outside box.
			AlignedBox< float, 3 > outsideBox( Vector3f( -2.f, -0.1f, 7.73726f ),
											   Vector3f( -1.f, 0.2f, 9.25754 ) ); 
			ASSERT_TRUE( frustum.isCullable( outsideBox ) );
			
			// 2nd case: intersecting box.
			AlignedBox< float, 3 > intersectingBox( Vector3f( 13.5f, -3.f, -15.f ),
													Vector3f( 15.f, 0.f, -5.f ) ); 
			ASSERT_FALSE( frustum.isCullable( intersectingBox ) );
			
			// 3rd case: inside box.
			AlignedBox< float, 3 > insideBox( Vector3f( 14.f, -1.f, -15.f ),
											  Vector3f( 15.f, 0.f, -14.f ) ); 
			ASSERT_FALSE( frustum.isCullable( insideBox ) );
		}
	}
}