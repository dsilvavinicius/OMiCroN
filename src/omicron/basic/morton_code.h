...
#include "omicron/memory/memory_utils.h"
...
namespace omicron::basic
{
	...
	ostream& operator<<( ostream& out, const MortonCode<Precision>& code )
	{
		...
		const vector< Precision > decoded = code.decode(level);
		out << "MortonCode: " << endl << "level = " << level << endl
			<< "coords = " << decoded << endl << code.getPathToRoot( true ) << endl;
        
//         out << "MortonCode: " << endl << "level = " << level << endl
// 			<< "coords = ";
//         omicron::util::operator<<( out, decoded );
//         out << endl << code.getPathToRoot( true ) << endl;
...
{
	template<>
    struct hash< omicron::basic::ShallowMortonCode >
    {
        size_t operator()( const omicron::basic::ShallowMortonCode& k ) const
        {
	...
	template<>
    struct hash< omicron::basic::MediumMortonCode >
    {
        size_t operator()( const omicron::basic::MediumMortonCode& k ) const
        {
